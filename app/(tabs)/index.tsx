...
function NannyCamTab({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  return (
    <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.card}>
        <View style={styles.alertHeaderRow}>
          <Text style={styles.cardTitle}>Manage Known Faces</Text>
        </View>
        <Text style={{fontSize: 14, color: "#6B7280", marginBottom: 12}}>
          Add photos of known adults to prevent "Unknown Person" alerts. If the camera is running, you must restart it to apply changes.
        </Text>

        {toastMessage && (
          <View style={{ backgroundColor: toastMessage.startsWith("✅") ? "#D1FAE5" : "#FEE2E2", borderColor: toastMessage.startsWith("✅") ? "#10B981" : "#EF4444", borderWidth: 1, padding: 12, borderRadius: 8, marginBottom: 14 }}>
            <Text style={{ color: toastMessage.startsWith("✅") ? "#065F46" : "#991B1B", fontWeight: "600", textAlign: "center", fontSize: 14 }}>
              {toastMessage}
            </Text>
          </View>
        )}
